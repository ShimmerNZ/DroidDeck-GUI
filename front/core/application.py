    ...
    def _toggle_failsafe(self):
        ...
        if state:
            ...
            if os.path.exists(pressed_icon):
                sender.setIcon(QIcon(pressed_icon))
        else:
            normal_icon = "resources/icons/failsafe.png"
            if os.path.exists(normal_icon):
                sender.setIcon(QIcon(normal_icon))
        
        # Send command to backend
        self.websocket.send_command("failsafe", state=state)
        self.logger.info(f"Failsafe toggled: {state}")
