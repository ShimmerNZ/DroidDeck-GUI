...
class SteamDeckControllerThread(QThread):
    ...
    def __init__(self, websocket_manager=None):
        ...
        # Reload imu_toggle_button mappings automatically whenever a config
        # save completes — registered with the central dispatcher so this
        # thread never parses unrelated traffic.
    ...
    def _read_and_send(self, current_time: float):
        ...
        # Skip frames identical to the last one sent - at idle (deadzoned
        # sticks, no buttons) this drops almost all of the 50Hz traffic.
        # A keepalive frame still goes out once per second so the backend
        # and IMU-presence detection continue to see a live controller.
        if (axes == self._last_sent_axes
