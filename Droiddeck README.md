...
- **Controller Integration**: Steam Deck gamepad mapping with calibration, differential steering, and IMU tilt control
...
# Steam Deck IMU tilt control (required for head servo tilt via controller gyro)
...
│   ├── application.py
│   ├── config_manager.py
│   ├── deck.py                # Steam Deck HID reader with IMU/accel support
│   ├── logger.py
...
DroidDeck supports using the Steam Deck's built-in accelerometer to drive head servos — tilt the controller to move the robot's head. This uses absolute gravity-relative tilt (not gyro rate integration), so the servo holds position when the controller is held still.

**How it works:** A custom HID reader (`core/deck.py`) opens the Steam Deck controller directly and sends a firmware command that keeps raw accelerometer and gyroscope data in the HID report. This command is re-sent every 800ms to counter Steam periodically disabling it, meaning IMU tilt works in both Desktop Mode and Gaming Mode with the gyro layout setting left at None.

**Setup:**
1. Assign IMU toggle buttons in the Controller settings screen
2. Press the assigned button combination to enable IMU tilt
3. The current controller angle is captured as the neutral/centre position on enable
4. Tilt the Deck left/right to drive the mapped head servos

**Dependencies:** `hid==1.0.6` and `scipy` must be installed (the automated installer handles this).

**Note:** `hid==1.0.6` specifically is required — the `hidapi` package uses a different API (`hid.device` lowercase) and will not work.
...
**IMU Tilt Not Working:**
- Verify `hid==1.0.6` is installed (not `hidapi`): `pip show hid`
- Verify `scipy` is installed: `pip show scipy`
- Check the log after toggling IMU — look for `accel zero: x=0 y=0` which means the firmware enable packet failed
- Ensure `core/deck.py` is present in the DroidDeck directory
- The gyro layout setting in Steam can be left at None — no configuration required
